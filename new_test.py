...
"""
我们还将实施一个略有修改的版本
该版本将使用指数移动平均而不是辅助损失来更新嵌入向量
这样做的优点是嵌入更新独立于编码器、解码器和架构其他部分的优化器选择
对于大多数实验
EMA版本的训练速度比非EMA版本快。
"""
class VectorQuantizerEMA(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, decay, eps=1e-5):
...
"""
train_res_recon_error_smooth = np.clip(train_res_recon_error_smooth, 0, 1)
train_res_perplexity_smooth = np.clip(train_res_perplexity_smooth, 0, 1)
"""
...
(outcome_originals, _) = next(iter(test_loader))
